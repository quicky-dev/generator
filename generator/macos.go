...
import (
	...
	"github.com/quicky-dev/generator/support/macos"
	"github.com/quicky-dev/generator/util"
	"strings"
)

type MACOS_GENERATOR struct {
...
func getSupportedMacPackages() SupportedPackages {
...
// to run locally
func (generator MACOS_GENERATOR) GenerateGenericScript() (Script, error) {
	if filePath == "" {
...
func (generator MACOS_GENERATOR) GenerateDynamicScript(install InstallRequest) (Script, error) {
	script := []string{}
	script = append(script, "#!/bin/bash\n")
...
func GetMacOSGenerator() MACOS_GENERATOR {
	...
	return MACOS_GENERATOR{getSupportedMacPackages(), genericMacPkgs}
}
